...
{
    [Parse setApplicationId:@"b7tbTQY9us1n6QqX82h3cRbMz0bfvacYDlW30YmJ"
                  clientKey:@"ounx8UMJVrtcrJL8N1AEhGnUyB9C4RWabJEOEkfK"];
...
- (void)showErrorAndLogout{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Login failed" message:@"Please try again" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
    [alertView show];
    [PFUser logOut];
}
...
- (void)facebookRequestDidLoad:(id)result{
    ...
    if (user) {
        // update current user with facebook and id
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection{
    if (self.profilePictureData.length == 0 || !self.profilePictureData) {
        ...
        // 注意：这里需要save两次，因为PFUser 不会自动save子对象
